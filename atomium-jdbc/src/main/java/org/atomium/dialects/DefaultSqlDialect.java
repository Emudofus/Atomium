...
import org.atomium.criterias.CriteriaPrinter;
import org.atomium.criterias.SqlCriteriaPrinter;
...
public class DefaultSqlDialect implements SqlDialectInterface {
    ...
    protected CriteriaPrinter printer() {
        return new SqlCriteriaPrinter();
    }
    ...
    <T> String columnStructure(Set<ColumnMetadata<T>> columns) {
        ...
        for (ColumnMetadata<T> column : columns) {
            ...
            Set<ColumnInfo> infos;
            ConverterInterface converter = column.getConverter();
            if (converter != null) {
                infos = converter.getBuiltStructure(column);
            } else {
                infos = ImmutableSet.of(column.asColumnInfo());
            }

            for (ColumnInfo info : infos) {
                JdbcDatabaseMetadata.SqlType sqlType = meta.getType(info.getTarget().getRawType());

                builder.append(escape(info.getName()))
    ...
    String printCriteria(CriteriaInterface criteria) {
        return printer().print(criteria);
    }
    ...
    @Override
    public <T> SqlQuery read(Metadata<T> meta, CriteriaInterface criteria) {
